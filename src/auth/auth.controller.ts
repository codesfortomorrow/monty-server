...
import { MFAVerifyRequestDto } from './dto/mfa-verify.request';
...
export class AuthController extends BaseController {
  ...
  async login(
    ...
    const response = await this.authService.signIn(req.user.id, req.user.type);
    if (response.success && response.accessToken) {
      this.setAuthCookie(res, response.accessToken, response.type);
    }
    return response;
  }
  ...
  async resultLogin(
    ...
    const role = await this.userService.getRoleByUserId(req.user.id);
    if (!role || role.name !== 'RESULT MANAGER')
      throw new Error("You don't have permission to login");
    const response = await this.authService.signIn(
      req.user.id,
      UserType.ResultManager,
    );
    if (response.success && response.accessToken) {
      this.setAuthCookie(res, response.accessToken, response.type);
    }
  ...
  async verifyMfa(
    ...
    const { verified, userID, userType } =
      await this.authService.verifyMfaToken(body.username, body.token);
    if (!verified || !userID || !userType)
  ...
  @Post('/demo/login')
  async demoLogin() {
    return await this.authService.demoLogin();
  }

  @Post('/dummy/login')
  async dummyLogin100() {
    return await this.authService.dummyLogin100();
  }
}

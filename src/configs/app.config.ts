...
export const appConfigFactory = registerAs<AppConfig>('app', () => ({
  ...
  userTypes: {
    OWNER: { level: 1, description: 'Full control over the platform' },
    'SUPER ADMIN': {
      level: 2,
      description: 'Manages most platform operations',
    },
    ADMIN: { level: 3, description: 'Limited administrative access' },
    'SUPER MASTER': { level: 4, description: 'Manages masters' },
    MASTER: { level: 5, description: 'Manages users' },
    USER: { level: 6, description: 'END USER' },
    DEMO: { level: 7, description: 'DEMO USER' },
    'RESULT MANAGER': {
      level: 8,
